...
namespace SysBot.AnimalCrossing
{
    ...
    {
        public static T ToStructure<T>(this byte[] bytes) where T : new()
        {
            var handle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            try
            {
                var obj = Marshal.PtrToStructure(handle.AddrOfPinnedObject(), typeof(T));
                return obj is T x ? x : new T();
            }
            finally
            {
                handle.Free();
        ...
        public static T ToClass<T>(this byte[] bytes) where T : new()
        {
            var handle = GCHandle.Alloc(bytes, GCHandleType.Pinned);
            try
            {
        ...
        public static T ToClass<T>(this byte[] bytes, int offset, int length) where T : class, new()
        {
            var slice = bytes.Slice(offset, length);
        ...
        public static T[] GetArray<T>(this byte[] data, int size) where T : class, new()
        {

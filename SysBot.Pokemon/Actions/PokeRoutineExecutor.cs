...
namespace SysBot.Pokemon
{
    public abstract class PokeRoutineExecutor<T> : PokeRoutineExecutorBase where T : PKM, new()
    {
        ...
        public abstract Task<T> ReadPokemon(uint offset, CancellationToken token);
        public abstract Task<T> ReadPokemon(uint offset, int size, CancellationToken token);
        public abstract Task<T> ReadBoxPokemon(int box, int slot, CancellationToken token);

        public async Task<T?> ReadUntilPresent(uint offset, int waitms, int waitInterval, int size, CancellationToken token)
        {
            int msWaited = 0;
